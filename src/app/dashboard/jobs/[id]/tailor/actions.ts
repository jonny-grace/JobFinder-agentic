...
const ai = new GoogleGenAI({ apiKey: process.env.GOOGLE_AI_API_KEY_USER! });
...
export async function generateTailoredResume(jobId: string) {
  ...
  const rewritePrompt = `
    ...
    Your task is to rewrite the content of the resume to target the job: ${
      job.title
    } at ${job.company}.
  ...
  const response = JSON.parse(
    rewriteResult.candidates?.[0]?.content?.parts?.[0]?.text || "{}"
  );
  ...
  const scoreData = JSON.parse(
    scoreResult.candidates?.[0]?.content?.parts?.[0]?.text || "{}"
  );
